...
type LocalPaymentMethod = 'cash' | 'mayar'
...
const paymentOptions: PaymentOption[] = [
  ...
  },
  {
    id: 'mayar',
    name: 'Mayar',
    description: 'Bayar via Mayar.id',
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      if (paymentMethod === 'mayar') {
        // Buka window dulu saat masih dalam user gesture context
        // agar tidak diblokir browser popup blocker
        const mayarWindow = window.open('', '_blank')

        const paymentResponse = await fetch('/api/payment/create', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ orderId, gateway: 'mayar' }),
        })

        if (!paymentResponse.ok) {
          mayarWindow?.close()
          throw new Error('Failed to create Mayar payment')
        }

        const { payment_url } = await paymentResponse.json()
        clearCart()
        if (mayarWindow) {
          mayarWindow.location.href = payment_url
        } else {
